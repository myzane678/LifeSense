...
public class AGConnectAuthUtils {
    ...
    public static OnFailureListener authFailureListener(final Result result) {
        return e -> {
            ...
            Map<String, Object> currentUser = mapFromUser(AGConnectAuth.getInstance().getCurrentUser());
            if (currentUser != null && e.getMessage() != null && e.getMessage().contains("InstantiationException")) {
    ...
    private static AGConnectAuthCredential email(Map credential) {
        ...
        if (verifyCode == null || verifyCode.isEmpty()) {
            return EmailAuthProvider.credentialWithPassword(email, password);
        }

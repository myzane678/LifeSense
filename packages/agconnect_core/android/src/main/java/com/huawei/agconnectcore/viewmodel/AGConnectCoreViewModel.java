    ...
    private void initAGConnectInstance() {
        if (AGConnectInstance.getInstance() == null) {
            AGConnectOptionsBuilder builder = new AGConnectOptionsBuilder()
                    ...
                    .setCustomValue("/agcgw/backurl", "connect-drcn.hispace.hicloud.com");
            try {
                InputStream inputStream = mActivity.getApplicationContext().getAssets().open("agconnect-services.json");
                builder.setInputStream(inputStream);
            } catch (IOException e) {
                Log.w("AGConnectCore", "agconnect-services.json asset not found.");
            }
            AGConnectInstance.initialize(mActivity.getApplicationContext(), builder);
        }
    }
